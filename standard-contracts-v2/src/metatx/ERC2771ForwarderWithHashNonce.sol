...
import { IForwarderSystem } from "@eveworld/world-core-v2/src/codegen/world/IForwarderSystem.sol";
...
contract ERC2771Forwarder is EIP712, HashNonce {
   ...
   */
  function structHash(ForwardRequest calldata request) public view returns (bytes32) {
    return
      _hashTypedDataV4(
        keccak256(
          abi.encode(
  ...
  function _isTrustedByTarget(address target) private returns (bool) {
    ...
    bytes memory encodedParams = abi.encodeCall(IForwarderSystem.evefrontier__isTrustedForwarder, (address(this)));
