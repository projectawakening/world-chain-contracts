...
/**
 * @title WorldWithEntryContext Contract
 * @author MUD (https://mud.dev) by Lattice (https://lattice.xyz)
 * @dev This contract is a custm core "World" contract containing various methods for
 * data manipulation, system calls, and dynamic function selector handling. And additionlly 
 * logic for capturing the initial msg.sender value from the entry call.
 *
 * @dev World doesn't inherit `Store` because the `IStoreRegistration` methods are added via the `InitModule`.
 */
contract WorldWithEntryContext is StoreKernel, IWorldKernel {
  ...
  /**
   * @dev Transaction entry context capture mutex
   */
  modifier entryContext(bool isCallFrom, address from) {
  ...
  ) external payable virtual prohibitDirectCallback entryContext(false, msg.sender) returns (bytes memory) {
  ...
  function callFrom(
    ...
    ResourceId systemId,
    bytes memory callData
  ) external payable virtual prohibitDirectCallback entryContext(true, delegator) returns (bytes memory) {
   ...
   */
  fallback() external payable prohibitDirectCallback entryContext(false, msg.sender) {
