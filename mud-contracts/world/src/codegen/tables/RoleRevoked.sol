...
library RoleRevoked {
   ...
   */
  function set(ResourceId _tableId, bytes32 roleId, RoleRevokedData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(ResourceId _tableId, bytes32 roleId, RoleRevokedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.account, _table.sender);
  ...
  function decode(
    ...
    PackedCounter,
    bytes memory
  ) internal pure returns (RoleRevokedData memory _table) {
