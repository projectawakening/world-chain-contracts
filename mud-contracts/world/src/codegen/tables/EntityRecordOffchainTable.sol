...
library EntityRecordOffchainTable {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "eveworld", name: "EntityRecordOffc", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x7462657665776f726c64000000000000456e746974795265636f72644f666663);
   ...
   */
  function get(uint256 entityId) internal view returns (EntityRecordOffchainTableData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(uint256(entityId));
   ...
   * @notice Get the full data.
   */
  function _get(uint256 entityId) internal view returns (EntityRecordOffchainTableData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(uint256(entityId));
   ...
   */
  function set(uint256 entityId, EntityRecordOffchainTableData memory _table) internal {
    bytes memory _staticData;
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(uint256 entityId, EntityRecordOffchainTableData memory _table) internal {
    bytes memory _staticData;
    EncodedLengths _encodedLengths = encodeLengths(_table.name, _table.dappURL, _table.description);
    bytes memory _dynamicData = encodeDynamic(_table.name, _table.dappURL, _table.description);
