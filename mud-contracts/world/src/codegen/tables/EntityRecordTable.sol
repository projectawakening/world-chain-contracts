...
library EntityRecordTable {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "eveworld", name: "EntityRecordTabl", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x7462657665776f726c64000000000000456e746974795265636f72645461626c);
   ...
   */
  function get(uint256 entityId) internal view returns (EntityRecordTableData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(uint256(entityId));
   ...
   * @notice Get the full data.
   */
  function _get(uint256 entityId) internal view returns (EntityRecordTableData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(uint256(entityId));
   ...
   */
  function set(uint256 entityId, EntityRecordTableData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(uint256 entityId, EntityRecordTableData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.itemId, _table.typeId, _table.volume, _table.recordExists);
