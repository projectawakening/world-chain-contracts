...
contract SmartGateSystem is EveSystem, AccessModified {
  ...
  function createAndAnchorSmartGate(
    ...
    SmartGateConfigTable.setMaxDistance(smartGateId, maxDistance);
  }
  ...
  function configureSmartGate(
    ...
    SmartGateConfigTable.setSystemId(smartGateId, systemId);
  }
  ...
  function isGateLinked(uint256 sourceGateId, uint256 destinationGateId) public view returns (bool) {
    return (
      (SmartGateLinkTable.getIsLinked(sourceGateId, destinationGateId)) ||
        (SmartGateLinkTable.getIsLinked(destinationGateId, sourceGateId))
        ? true
        : false
    );
  }
