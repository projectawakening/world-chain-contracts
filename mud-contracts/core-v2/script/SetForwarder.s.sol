...
contract SetForwarder is Script {
  function run(address worldAddress) external {
    ...
    IWorld(worldAddress).evefrontier__setTrustedForwarder(trustedForwarderAddress);

    console.log("TrustedForwarder: ");
    console.logAddress(trustedForwarderAddress);
    console.log(IWorld(worldAddress).evefrontier__isTrustedForwarder(trustedForwarderAddress));
