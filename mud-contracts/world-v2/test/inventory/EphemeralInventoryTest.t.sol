...
contract MockInventoryInteractSystem is System {
  ...
  function callEphemeralDeposit(uint256 smartObjectId, address ephemeralOwner, InventoryItemParams[] memory items) public {
    ephemeralInventorySystem.depositEphemeral(smartObjectId, ephemeralOwner, items);
  }
  
  function callEphemeralWithdraw(uint256 smartObjectId, address ephemeralOwner, InventoryItemParams[] memory items) public {
...
contract EphemeralInventoryTest is MudTest {
  ...
  vm.startPrank(deployer);
    ...
    vm.expectRevert(abi.encodeWithSelector(EphemeralInventorySystem.EphemeralInventory_InvalidTenantId.selector, testItems[0].smartObjectId, testItems[0].tenantId));
    ...
    vm.expectRevert(abi.encodeWithSelector(EphemeralInventorySystem.EphemeralInventory_InvalidItemObjectId.selector, testItems[0].smartObjectId));
    ephemeralInventorySystem.createAndDepositEphemeral(inventoryObjectId, bob, testItems);
    ...
    vm.expectRevert(abi.encodeWithSelector(EphemeralInventorySystem.EphemeralInventory_InvalidItemDepositQuantity.selector, singletonObjectId, 2));
    ephemeralInventorySystem.createAndDepositEphemeral(inventoryObjectId, bob, testItems);
    ...
    vm.startPrank(bob, deployer);
    vm.expectRevert(abi.encodeWithSelector(DeployableSystem.Deployable_IncorrectState.selector, inventoryObjectId, State.ANCHORED));
    ...
    assertEq(EphemeralInventory.getCapacity(inventoryObjectId, bob), EphemeralInvCapacity.getCapacity(inventoryObjectId));
  ...
  function testDepositEphemeral() public {
    ...
    deployableSystem.anchor(inventoryObjectId, alice, LocationData({
    ...
    newItemParams[0] = InventoryItemParams({
      smartObjectId: item1ObjectId,
      quantity: 1
    });
    
    newItemParams[1] = InventoryItemParams({
      smartObjectId: item2ObjectId,
      quantity: 3
    });
    
    newItemParams[2] = InventoryItemParams({
    ...
    EphemeralInvItemData memory newTransferItemData = EphemeralInvItem.get(inventoryObjectId, bob, transferItemObjectId);
  ...
  function testWithdrawEphemeral() public {
    ...
    EphemeralInvItemData memory transferItemObjectData = EphemeralInvItem.get(inventoryObjectId, bob, transferItemObjectId);
  ...
  function test_EphemeralInventory_EphemeralToInventoryTransfer() public {
    ...
    mainInventoryItems[0] = InventoryItemParams({
      smartObjectId: item1ObjectId,
      quantity: 1
    });
    mainInventoryItems[1] = InventoryItemParams({
      smartObjectId: item2ObjectId,
      quantity: 4
    });
    mainInventoryItems[2] = InventoryItemParams({
      smartObjectId: transferItemObjectId,
      quantity: 5
    });
    ...
    _simulateToEphemeralTransferCall(
      inventoryObjectId,
      bob,
      toEphemeralItems
    );
    ...
    vm.prank(bob, deployer);
    _simulateFromEphemeralTransferCall(
      inventoryObjectId,
      bob,
      toMainItems
    );
  ...
  function test_EphemeralInventory_CrossEphemeralTransfer() public {
    ...
    bobItems[2] = InventoryItemParams({
      smartObjectId: transferItemObjectId,
      quantity: 5
    });
