...
contract EntityRecordTest is MudTest {
  ...
  bytes32 tenantId = bytes32("TENANT");
  ...
  function testEntityRecord(uint256 itemId, uint256 typeId, uint256 volume) public {
    ...
    EntityRecordParams memory entityRecordParams = EntityRecordParams({ tenantId: tenantId, typeId: typeId, itemId: itemId, volume: volume });
  ...
  function testEntityRecordMetadata() public {
    vm.startPrank(deployer);
    entitySystem.instantiate(testClassId, smartObjectId, deployer);
    EntityMetadataParams memory entityMetadataParams = EntityMetadataParams({ name: name, dappURL: dappURL, description: description });

    entityRecordSystem.createMetadata(smartObjectId, entityMetadataParams);
