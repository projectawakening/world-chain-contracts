...
library KillMail {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "evefrontier", name: "KillMail", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x746265766566726f6e746965720000004b696c6c4d61696c0000000000000000);
   ...
   */
  function get(uint256 killMailId) internal view returns (KillMailData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(uint256(killMailId));
   ...
   * @notice Get the full data.
   */
  function _get(uint256 killMailId) internal view returns (KillMailData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(uint256(killMailId));
   ...
   */
  function set(uint256 killMailId, KillMailData memory _table) internal {
    bytes memory _staticData = encodeStatic(
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(uint256 killMailId, KillMailData memory _table) internal {
    bytes memory _staticData = encodeStatic(
      _table.killerCharacterId,
      _table.victimCharacterId,
      _table.lossType,
  ...
  function decode(
    ...
    EncodedLengths,
    bytes memory
  ) internal pure returns (KillMailData memory _table) {
    (
      _table.killerCharacterId,
      _table.victimCharacterId,
      _table.lossType,
