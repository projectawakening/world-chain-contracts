...
struct SmartGateConfigData {
  ResourceId systemId;
  uint256 maxDistance;
}

library SmartGateConfig {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "evefrontier", name: "SmartGateConfig", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x746265766566726f6e74696572000000536d61727447617465436f6e66696700);
   ...
   */
  function get(uint256 smartObjectId) internal view returns (SmartGateConfigData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(uint256(smartObjectId));
   ...
   * @notice Get the full data.
   */
  function _get(uint256 smartObjectId) internal view returns (SmartGateConfigData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(uint256(smartObjectId));
   ...
   */
  function set(uint256 smartObjectId, SmartGateConfigData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(uint256 smartObjectId, SmartGateConfigData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.systemId, _table.maxDistance);
  ...
  function decodeStatic(bytes memory _blob) internal pure returns (ResourceId systemId, uint256 maxDistance) {
    ...
    maxDistance = (uint256(Bytes.getBytes32(_blob, 32)));
  }
  ...
  function decode(
    ...
    EncodedLengths,
    bytes memory
  ) internal pure returns (SmartGateConfigData memory _table) {
